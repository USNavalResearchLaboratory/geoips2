...
def produce_all_sector_metadata(final_products, area_def, xarray_obj, metadata_dir='metadata'):
    ...
    if is_sector_type(area_def, 'atcf'):
        for final_product in final_products:
            ...
            yaml_products += produce_sector_metadata(area_def, xarray_obj, final_product, metadata_dir=metadata_dir)
...
def output_metadata_yaml(metadata_fname, area_def, xarray_obj, productname=None):
    ...
    if productname:
        sector_info['product_filename'] = productname
    if 'original_source_filename' in xarray_obj.attrs.keys():
        from os.path import basename
        sector_info['original_source_filename'] = basename(xarray_obj.original_source_filename)
