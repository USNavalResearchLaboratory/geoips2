...
def is_valid_filenamer(filename_func_name):
    ...
    Check that the requested filename function has the correct call signature.
        ...
        Filename functions currently defined in:
            <geoips2_package>.filenames.<filename_func_name>.<filename_func_name>
        ...
        Output type currently found in:
            <geoips2_package>.filenames.<filename_func_name>.filename_type
...
def get_remove_duplicates_func(filename_func_name):
    ...
    See: geoips2.dev.filename.is_valid_filenamer for full list of supported filename function call signatures and return values
...
def list_filenamers_by_type():
    ...
    See geoips2.dev.filename.is_valid_filenamer? for a list of available filename format types and associated call signatures / return values.
    See geoips2.dev.filename.get_filenamer(filename_func_name) to retrieve the requested filename format function
...
def test_filename_interface():
    ''' Finds and opens every filename func available within the current geoips2 instantiation

    See geoips2.dev.filename.is_valid_filenamer? for a list of available filename func types and associated call signatures / return values.
    See geoips2.dev.filename.get_filenamer(filename_func_name) to retrieve the requested filename func

    Returns:
        (list) : List of all successfully opened geoips2 filename funcs
    '''
