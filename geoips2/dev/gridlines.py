...
''' Interface Under Development.  Please provide feedback to geoips@nrlmry.navy.mil
    ...
    Wrapper functions for geoips2 gridlines plotting parameter specifications
...
from geoips2.geoips2_utils import find_config
...
def is_valid_gridlines(gridlines_name):
    ...
    optional_keys = {'standard': ['grid_lat_xoffset',
                                  'grid_lon_xoffset',
                                  'grid_lat_yoffset',
                                  'grid_lon_yoffset',
                                  'grid_lat_fontsize',
...
def get_gridlines(gridlines_name):
    ...
    See: geoips2.dev.gridlines.is_valid_gridlines for full list of supported gridlines dictionary formats
    ...
    if gridlines_name not in gridlines_dict:
        raise ValueError(f'gridlines file {gridlines_fname} must contain gridlines name {gridlines_name} as key')
...
def get_gridlines_type(gridlines_name):
    ...
    Retrieve gridlines_dict_type of the requested gridlines, found in:
           geoips2.dev.gridlines.get_gridlines(gridlines_name)['gridlines_dict_type']

    See: geoips2.dev.gridlines.is_valid_gridlines for full list of supported gridlines dict types.
...
def list_gridlines_by_type():
    ...
    See geoips2.dev.gridlines.is_valid_gridlines? for a list of available gridlines types and associated dictionary formats.
    See geoips2.dev.gridlines.get_gridlines(gridlines_name) to retrieve the gridlines parameter dictionary
    ...
    from geoips2.geoips2_utils import list_gridlines_params_dict_yamls
...
def test_gridlines_interface():
    ''' Finds and opens every gridlines params dict available within the current geoips2 instantiation

    See geoips2.dev.gridlines.is_valid_gridlines? for a list of available gridlines params dict types and associated call signatures / return values.
    See geoips2.dev.gridlines.get_gridlines(gridlines_params_dict_name) to retrieve the requested gridlines params dict

    Returns:
        (list) : List of all successfully opened geoips2 gridlines params dicts
    '''
