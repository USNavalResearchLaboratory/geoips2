...
''' Interface Under Development.  Please provide feedback to geoips@nrlmry.navy.mil
    ...
    Wrapper functions for geoips2 METOCTIFF interfacing.
...
def is_valid_alg(alg_func_name):
    ...
    Algorithm func type currently found in
        <geoips2_package>.algorithms.*.<alg_func_name>.alg_params['alg_type']

    Algorithm functions currently defined in
        <geoips2_package>.algorithms.*.<alg_func_name>.<alg_func_name>
    and requested within the product parameters dictionary
        See geoips2.dev.product.check_product_params_dict
    ...
    For package based algorithm functions:
        See geoips2.dev.alg.get_alg
        See geoips2.dev.alg.get_alg_type
        See geoips2.dev.alg.list_algs_by_type
    
    For product based algorithm functions:
        See geoips2.dev.alg.get_alg
        See geoips2.dev.alg.get_alg_name
        See geoips2.dev.alg.get_alg_args
    '''
    ...
    if alg_func_type not in required_kwargs:
        raise ValueError(f'\nalg_func "{alg_func_name}" of unsupported type "{alg_func_type}"\n'
                         f'alg_func_type must be one of: {required_kwargs.keys()}\n'
                         f'Either add "{alg_func_type}" to list of supported types in\n'
                         f'geoips2.dev.alg.is_valid_alg.required_kwargs\n'
                         f'or update "{alg_func.__module__}" to a supported type')
...
def get_alg(alg_func_name):
    ...
    See: geoips2.dev.alg.is_valid_alg for full list of supported algorithm function call signatures and return values
...
def list_algs_by_type():
    ...
    See geoips2.dev.alg.is_valid_alg? for a list of available algorithm function types and associated call signatures / return values.
    See geoips2.dev.alg.get_alg(alg_func_name) to retrieve the requested algorithm function
    See geoips2.dev.alg.get_alg_type(alg_func_name) to retrieve the requested algorithm function type
...
def test_alg_interface():
    ''' Finds and opens every alg func available within the current geoips2 instantiation

    See geoips2.dev.alg.is_valid_alg? for a list of available alg func types and associated call signatures / return values.
    See geoips2.dev.alg.get_alg(alg_func_name) to retrieve the requested alg func

    Returns:
        (list) : List of all successfully opened geoips2 alg funcs
    '''
