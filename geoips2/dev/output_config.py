...
''' Interface Under Development.  Please provide feedback to geoips@nrlmry.navy.mil
    ...
    Wrapper functions for geoips2 output_config specifications.
...
def is_valid_output_config(output_config_dict):
    ...
    if output_config_dict['output_config_type'] not in required_keys:
        LOG.error("INVALID OUTPUT CONFIG '%s': 'output_config_type' in output config dictionary must be one of '%s'",
                  output_config_dict.keys(),
    ...
    if not set(required_keys[output_config_type]).issubset(set(output_config_dict)):
        LOG.error("INVALID OUTPUT CONFIG '%s': '%s' output config dictionary must contain the following fields: '%s'",
                  output_config_dict.keys(),
                  output_config_type,
    ...
    if not set(output_config_dict).issubset(required_keys[output_config_type]+optional_keys[output_config_type]):
        LOG.error("INVALID OUTPUT CONFIG'%s': Unknown fields in '%s' output config dictionary: '%s'",
                  output_config_dict.keys(),
                  output_config_type,
                  set(output_config_dict).difference(required_keys[output_config_type] +
                                                     optional_keys[output_config_type]))
        return False
...
def get_output_config_type(output_config_dict):
    ...
    See: geoips2.dev.output_config.is_valid_output_config for full list of supported output_config types.
...
def get_filename_formats(output_dict):
    if 'filename_format' in output_dict and isinstance(output_dict['filename_format'], str):
...
def get_metadata_filename_format(filename_format, output_dict):
    ...
    if 'metadata_filename_format' in output_dict and isinstance(output_dict['metadata_filename_format'], str):
        fname_format = output_dict['metadata_filename_format']

    elif 'metadata_filename_formats' in output_dict and filename_format in output_dict['metadata_filename_formats']:
        fname_format = output_dict['metadata_filename_formats'][filename_format]

    elif 'metadata_filename_formats' in output_dict and 'all' in output_dict['metadata_filename_formats']:
...
def get_minimum_coverage(product_name, output_dict):
    ...
    if 'minimum_coverage' in output_dict and output_dict['minimum_coverage'] is not None:
        minimum_coverage = float(output_dict['minimum_coverage'])

    elif 'minimum_coverages' in output_dict and product_name in output_dict['minimum_coverages']:
        minimum_coverage = float(output_dict['minimum_coverages'][product_name])

    elif 'minimum_coverages' in output_dict and 'all' in output_dict['minimum_coverages']:
        minimum_coverage = float(output_dict['minimum_coverages']['all'])

    if minimum_coverage is not None and (minimum_coverage < 0 or minimum_coverage > 100):
        raise ValueError(f'Minimum coverage must be percentage between 0 and 100, {minimum_coverage} invalid')
...
def get_filename_format_kwargs(filename_format, output_dict):
    ...
    if 'filename_formats_kwargs' in output_dict and 'all' in output_dict['filename_formats_kwargs']:
        filename_format_kwargs = output_dict['filename_formats_kwargs']['all']
    if 'filename_formats_kwargs' in output_dict and filename_format in output_dict['filename_formats_kwargs']:
...
def get_metadata_filename_format_kwargs(filename_format, output_dict):
    ...
    if 'metadata_filename_formats_kwargs' in output_dict and 'all' in output_dict['metadata_filename_formats_kwargs']:
        metadata_filename_format_kwargs = output_dict['metadata_filename_formats_kwargs']['all']

    if 'metadata_filename_formats_kwargs' in output_dict \
       and filename_format in output_dict['metadata_filename_formats_kwargs']:
        metadata_filename_format_kwargs = output_dict['metadata_filename_formats_kwargs'][filename_format]
...
def get_output_format_kwargs(output_dict, xarray_obj=None, area_def=None, sector_type=None,
    ...
    if bg_files and 'background_products' in output_dict and sector_type in bg_xarrays:
        ...
        output_format_kwargs['bg_data'] = output_format_kwargs['bg_xarray'][bg_product_name].to_masked_array()
        output_format_kwargs['bg_product_name_title'] = bg_product_name
        output_format_kwargs['bg_mpl_colors_info'] = None
        bg_cmap_func_name = get_cmap_name(bg_product_name,
...
def get_metadata_output_format_kwargs(output_dict):
    ...
    if 'metadata_output_format_kwargs' in output_dict:
        metadata_output_format_kwargs = output_dict['metadata_output_format_kwargs'].copy()
...
def produce_current_time(config_dict, metadata_xobj, output_dict_keys=None):
    ...
    for output_dict_key in output_dict_keys:
        ...
        if 'produce_times' not in output_dict:
            LOG.info('WILL PROCESS "produce_times" not included in output_dict: %s', output_dict_key)
            retval = True
        elif 'required_minutes' in output_dict['produce_times']:
            if metadata_xobj.start_datetime.minute in output_dict['produce_times']['required_minutes']:
                LOG.info('WILL PROCESS produce_times["required_minutes"]=%s includes current minute %s, %s',
...
def test_output_config_interface(output_config_dict):
    ...
    See geoips2.dev.output_config.is_valid_output_config? for a list of available product params dict types and associated call signatures / return values.
