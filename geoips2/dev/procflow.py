...
''' Interface Under Development.  Please provide feedback to geoips@nrlmry.navy.mil
    ...
    Wrapper functions for geoips2 procflow interface.
...
def is_valid_procflow(procflow_func_name):
    ...
    Check that the requested procflow function has the correct call signature.
        ...
        Driver functions currently defined in
            <geoips2_package>.procflows.<procflow_func_name>.<procflow_func_name>
...
def get_procflow(procflow_func_name):
    ...
    See: geoips2.dev.procflow.is_valid_procflow for full list of supported procflow function call signatures and return values

    Args:
        procflow_func_name (str) : Desired procflow function (ie, 'single_source', 'pmw_tbs', 'stitched_procflow', etc)

    Returns:
        (<procflow function>) : Driver function, currently specified in <geoips2_package>.procflows.<procflow_func_name>.<procflow_func_name>
    '''
...
def get_procflow_type(procflow_func_name):
    ...
    Returns:
        (str) : Driver type currently found in <geoips2_package>.procflows.<procflow_func_name>.procflow_type
...
def list_procflows_by_type():
    ...
    See geoips2.dev.procflow.is_valid_procflow? for a list of available procflow types and associated call signatures / return values.
    Use geoips2.dev.procflow.get_procflow(procflow_func_name) to retrieve the requested procflow
...
def test_procflow_interface():
    ''' Finds and opens every procflow func available within the current geoips2 instantiation

    See geoips2.dev.procflow.is_valid_procflow? for a list of available procflow func types and associated call signatures / return values.
    See geoips2.dev.procflow.get_procflow(procflow_func_name) to retrieve the requested procflow func

    Returns:
        (list) : List of all successfully opened geoips2 procflow funcs
    '''
