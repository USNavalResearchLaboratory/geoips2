...
''' Interface Under Development.  Please provide feedback to geoips@nrlmry.navy.mil
    ...
    Wrapper functions for geoips2 interpolation functionality.
...
def is_valid_interp(interp_func_name):
    ...
    Check that the requested interpolation function has the correct call signature.
        Return values should be as specified below, but are not programmatically verified.

    Args:
        interp_func_name (str) : Desired interp function (ie, 'interp_nearest', 'interp_gauss', etc)
    ...
    Interpolation function type currently found in
        <geoips2_package>.interpolation.*.<interp_func_name>.interp_type

    Interpolation functions currently defined in
        <geoips2_package>.interpolation.*.<interp_func_name>.<interp_func_name>
    and requested within the product parameters dictionary
        See geoips2.dev.product.check_product_params

    interp_func types currently one of:

           '2d' : call signature <interp_func_name>(area_def, xarray_obj, varlist, array_num=None, **kwargs)
                        return value: mpl_colors_info dict
           'grid' : call signature <interp_func_name>(area_def, xarray_obj, varlist, array_num=None, **kwargs)
                        return value: mpl_colors_info dict

    Return list of arrays (same length and order as "varlist" argument)

    For package based interpolation functions:
        See geoips2.dev.interp.get_interp
        See geoips2.dev.interp.get_interp_type
        See geoips2.dev.interp.list_interps_by_type
    
    For product based interpolation functions:
        See geoips2.dev.interp.get_interp
        See geoips2.dev.interp.get_interp_name
        See geoips2.dev.interp.get_interp_args
    '''
    required_args = {'2d': ['area_def', 'input_xarray', 'output_xarray', 'varlist'],
                     'grid': ['area_def', 'input_xarray', 'output_xarray', 'varlist']}
    required_kwargs = {'2d': ['array_num'],
                       'grid': ['array_num']}
    ...
    if not set(required_args[interp_func_type]).issubset(set(interp_func_args)):
        LOG.error("INVALID INTERP FUNC '%s': '%s' interp func type must have required arguments: '%s'",
                  interp_func_name, interp_func_type, required_args[interp_func_type])
        return False
    ...
    if not set(required_kwargs[interp_func_type]).issubset(set(interp_func_kwargs)):
        LOG.error("INVALID INTERP FUNC '%s': '%s' interp_func type must have kwargs: '%s'",
                  interp_func_name, interp_func_type, required_kwargs[interp_func_type])
        return False
...
def get_interp(interp_func_name):
    ...
    See: geoips2.dev.interp.is_valid_interp for full list of supported interp function call signatures and return values

    Args:
        interp_func_name (str) : Desired interp function (ie, 'interp_nearest', 'interp_gauss', etc)

    Returns:
        (<interp function>) : Interpolation function
    '''
...
def get_interp_type(interp_func_name):
    ...
    Returns:
        (str) : Interpolation function type
    '''
...
def list_interps_by_type():
    ...
    List all available interpolation functions within the current GeoIPS instantiation, sorted by interp_type

    Interpolation function "type" determines exact required call signatures and return values

    See geoips2.dev.interp.is_valid_interp? for a list of available interpolation function types and associated call signatures / return values.
    See geoips2.dev.interp.get_interp(interp_func_name) to retrieve the requested interpolation function
    See geoips2.dev.interp.get_interp_type(interp_func_name) to retrieve the requested interpolation function type

    Returns:
        (dict) : Dictionary with all interpolation function types as keys, and associated interpolation function names (str) as values.
    '''
...
def test_interp_interface():
    ''' Finds and opens every interp func available within the current geoips2 instantiation

    See geoips2.dev.interp.is_valid_interp? for a list of available interp func types and associated call signatures / return values.
    See geoips2.dev.interp.get_interp(interp_func_name) to retrieve the requested interp func

    Returns:
        (list) : List of all successfully opened geoips2 interp funcs
    '''
    ...
    # from geoips2.dev.product import list_products_by_source
