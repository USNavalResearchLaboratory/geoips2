...
if not getenv('GEOIPS2'):
    ...
    PATHS['GEOIPS2'] = getenv('GEOIPS2').rstrip('/')

if getenv('GEOIPS2_PACKAGES_DIR') and exists(getenv('GEOIPS2_PACKAGES_DIR')):
    PATHS['GEOIPS2_PACKAGES_DIR'] = getenv('GEOIPS2_PACKAGES_DIR').rstrip('/')
    PATHS['GEOIPS2_PACKAGES'] = listdir(getenv('GEOIPS2_PACKAGES_DIR'))
elif getenv('GEOIPS2_PACKAGES_DIR'):
    PATHS['GEOIPS2_PACKAGES_DIR'] = getenv('GEOIPS2_PACKAGES_DIR').rstrip('/')
    PATHS['GEOIPS2_PACKAGES'] = ['geoips2']
else:
    PATHS['GEOIPS2_PACKAGES_DIR'] = None
    PATHS['GEOIPS2_PACKAGES'] = ['geoips2']

if not getenv('GEOIPS2_BASEDIR'):
    PATHS['GEOIPS2_BASEDIR'] = pathjoin(PATHS['GEOIPS2_PACKAGES_DIR'], '..')
else:
    PATHS['GEOIPS2_BASEDIR'] = getenv('GEOIPS2_BASEDIR').rstrip('/')
...
if getenv('CLEAN_IMAGERY_PATH'):
    ...
    PATHS['CLEAN_IMAGERY_PATH'] = pathjoin(PATHS['GEOIPS_OUTDIRS'], 'preprocessed', 'clean_imagery')

# Location for writing out pregenerated "clean" imagery files
if getenv('ANNOTATED_IMAGERY_PATH'):
    PATHS['ANNOTATED_IMAGERY_PATH'] = getenv('ANNOTATED_IMAGERY_PATH').rstrip('/')
else:
    PATHS['ANNOTATED_IMAGERY_PATH'] = pathjoin(PATHS['GEOIPS_OUTDIRS'], 'preprocessed', 'annotated_imagery')

# Location for writing out precalculated data files (algorithms applied)
if getenv('FINAL_DATA_PATH'):
    PATHS['FINAL_DATA_PATH'] = getenv('FINAL_DATA_PATH').rstrip('/')
else:
    PATHS['FINAL_DATA_PATH'] = pathjoin(PATHS['GEOIPS_OUTDIRS'], 'preprocessed', 'final')

# Location for writing out pregenerated geolocation netcdf files
if getenv('PREGENERATED_GEOLOCATION_PATH'):
    PATHS['PREGENERATED_GEOLOCATION_PATH'] = getenv('PREGENERATED_GEOLOCATION_PATH').rstrip('/')
else:
...
PATHS['TC_TEMPLATE'] = pathjoin(PATHS['GEOIPS2'], 'geoips2', 'yaml_configs', 'sectors_dynamic', 'tc_web_template.yaml')
...
if getenv('PRIVATEWWW'):
    ...
    PATHS['PRIVATEWWW'] = pathjoin(PATHS['GEOIPS_OUTDIRS'], 'preprocessed', 'privatewww')
...
PATHS['TC_DECKS_DB'] = pathjoin(PATHS['GEOIPS_OUTDIRS'], 'longterm_files', 'tc', 'tc_decks.db')
...
if getenv('TC_DECKS_DIR'):
    ...
    PATHS['TC_DECKS_DIR'] = pathjoin(PATHS['GEOIPS_OUTDIRS'], 'longterm_files', 'tc', 'decks')
