...
'''
This reader is designed for geoips2 for importing MODIS data files in hdf4 format.  Example files are:
AQUA:      MYD files
...
# corrections[chan][2] = (tci1km) temperature correction intercept
        ...
        if cname == 'MOD03' or cname == 'MYD03':
            ...
            for datasettag in dataset_info.keys():                        # loop the data_type
                ...
                for currvar in scifile_names.keys():    # loop variables assocaited with geo-info
            ...
            add_to_xarray('fire_mask', masked_data, xarrays[datasettag], cumulative_mask[datasettag], datasettag)
            scifile_names = {'Latitude': 'latitude',
                             'Longitude': 'longitude', }
            ...
            for ii in range(len(datapaths)):
                ...
                for jj in range(len(chanlocs)):
                    ...
                    if channame in corrections.keys():
                        ...
                        # By default, GeoIPS2 readers should store BT data internally in Kelvin.
                    ...
                    if channame in\
                       corrections_ref[mf_metadata['ASSOCIATEDPLATFORMSHORTNAME'].lower()].keys():
                        ...
                        offset = corrections_ref[
                            mf_metadata['ASSOCIATEDPLATFORMSHORTNAME'].lower()][channame][1]
    ...
    for dtype in xarrays.keys():
        ...
        if dtype != 'METATDATA' and 'latitude' not in list(xarrays[dtype].variables.keys()):
            LOG.info('No data read for dataset %s, removing from xarray list', dtype)
            continue
        if dtype != 'METADATA':
            for varname in list(xarrays[dtype].variables.keys()):
                xarrays[dtype][varname] = xarrays[dtype][varname].where(~cumulative_mask[dtype][varname])
