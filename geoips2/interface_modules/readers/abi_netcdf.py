...
DATASET_INFO = {'MED': ['B01', 'B03', 'B05'],
                ...
                'LOW': ['B04', 'B06', 'B07', 'B08', 'B09', 'B10', 'B11', 'B12',
                        'B13', 'B14', 'B15', 'B16']}
ALL_GEO_VARS = ['SunZenith', 'SatZenith', 'SunAzimuth', 'SatAzimuth', 'latitude', 'longitude']
ALL_CHANS = {'LOW': ['B04Rad', 'B04Ref',   # 1.37um Near-IR Cirrus
                     ...
                     'B08Rad', 'B08BT',    # 6.2um  IR      Upper-level tropospheric water vapor
                     'B09Rad', 'B09BT',    # 6.9um  IR      Mid-level water vapor
...
def _get_geolocation_metadata(metadata):
    ...
    geomet['res_km'] = float(metadata['file_info']['spatial_resolution'].split()[0][0:-2])
...
def abi_netcdf(fnames, metadata_only=False, chans=None, area_def=None, self_register=False):
    ...
    if self_register:
        ...
        fldk_lats, fldk_lons = get_latitude_longitude(standard_metadata[adname], BADVALS, area_def)
        gvars[adname] = get_geolocation(sdt, standard_metadata[adname], fldk_lats, fldk_lons, BADVALS, area_def)
        if not gvars[adname]:
        ...
        for res in ['LOW', 'MED', 'HIGH']:
            ...
            try:
                ...
                fldk_lats, fldk_lons = get_latitude_longitude(standard_metadata[res], BADVALS, area_def)
                gvars[res] = get_geolocation(sdt, standard_metadata[res], fldk_lats, fldk_lons, BADVALS, area_def)
    ...
    for dsname in datavars.keys():
        ...
        if hasattr(xobj, 'area_definition') and xobj.area_definition is not None:
            roi = max(xobj.area_definition.pixel_size_x,
