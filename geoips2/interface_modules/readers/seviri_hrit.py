...
from geoips2.interface_modules.readers.utils.geostationary_geolocation import get_geolocation_cache_filename, get_geolocation, AutoGenError
...
def get_latitude_longitude(gmd, BADVALS, area_def):
    ...
    deg2rad = np.pi / 180.0
    x, y = np.meshgrid(np.arange(0, gmd['num_samples'], 1), np.arange(0, gmd['num_lines'], 1))
    x = np.fliplr(x)
    y = np.fliplr(y)
...
def _get_geolocation_metadata(prologue, file_metadata):
    ...
    geomet['scan_mode'] = prologue['imageDescription']['projectionDescription']['typeOfProjection']
    # Used for cached filenames
    geomet['scene'] = geomet['scan_mode']
    geomet['lon0'] = prologue['imageDescription']['projectionDescription']['longitudeOfSSP']
    geomet['num_lines'] = prologue['imageDescription']['referenceGridVIS_IR']['numberOfLines']
    geomet['num_samples'] = prologue['imageDescription']['referenceGridVIS_IR']['numberOfColumns']
    ...
    geomet['start_datetime'] = prologue['imageAcquisition']['plannedAcquisitionTime']['trueRepeatCycleStart']
...
class Chan(object):
    _good_names = ['B01Rad', 'B01Ref',  # VIS0.6 Cloud mapping
                   'B02Rad', 'B02Ref',  # VIS0.8 Vegetation index
                   'B03Rad', 'B03Ref',  # NIR1.6 Cloud / snow discrimination
                   'B04Rad', 'B04BT',   # IR3.9  Atmospheric window
                   'B05Rad', 'B05BT',   # IR6.2  Water vapour channel (Upper atmosphere)
                   'B06Rad', 'B06BT',   # IR7.3  Water vapour channel (Lower atmosphere)
                   'B07Rad', 'B07BT',   # IR8.7  Atmospheric window
                   'B08Rad', 'B08BT',   # IR9.7  Ozone channel
                   'B09Rad', 'B09BT',   # IR10.8 Atmospheric window
                   'B10Rad', 'B10BT',   # IR12.0 Atmospheric window
                   'B11Rad', 'B11BT']   # IR13.4 Carbon dioxide channel
...
def seviri_hrit(fnames, metadata_only=False, chans=None, area_def=None, self_register=False):
    ...
    for fname in fnames:
        ...
        if df.start_datetime != sdt:
            raise HritError('Start date time does not match for all files: {}'.format(fnames))
        # Get prologue
        ...
        # Store data files, organized by band, then segment
            ...
            if not compare_dicts(imgf.geolocation_metadata, df.geolocation_metadata,
                ...
                raise HritError(
                    'Geolocation metadata do not match for image files: {}'.format(fnames))
        ...
        if band not in chlist.bands:
            ...
            for seg, df in dfs[band].items():
                ...
                try:
                    ...
                    skip_segs += [seg]
                    LOG.error('FAILED DECOMPRESSING, %s, SKIPPING FILE %s', str(resp), df.name)
