...
def flat_sectorfile_parser(sectorfile_name):
    ...
    for line in flatsf_lines:
        if not line.strip():
            continue
        curr_fields = parse_flat_sectorfile_line(line, sectorfile_name, parser_name='flat_sectorfile_parser')
...
def parse_flat_sectorfile_line(line, source_filename, parser_name='flat_sectorfile_parser'):
    ...
    LOG.info(len(parts))
    if len(parts) < 8:
        raise ValueError('Incorrectly formatted flat text sectorfile - fewer than 8 fields')
