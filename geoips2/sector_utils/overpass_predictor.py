...
def calculate_overpass(tle, observer_lat, observer_lon, date):
    ...
    opass_info = {'check datetime': date,
                  'sunrise': sunrise,
                  'sunset': sunset,
    ...
    ptsatdeg = spherical_geometry.Coordinate(lon=opass_info['sublon'],
                                             lat=opass_info['sublat'])
...
def predict_overpass_area_def(tlefile, area_definition, satellite_list, start_datetime, check_midpoints=False):
...
def predict_overpass_yaml(tlefile, sectorfile, sector_list, satellite_list, start_datetime, check_midpoints=False):
    ...
    '''
    from geoips2.sector_utils.utils import create_areadefinition_from_yaml
    if not isinstance(satellite_list, list):
        raise TypeError('satellite_list must be type list')
    sector_overpasses = {}
    for yaml_sector in sector_list:
        sector_area_def = create_areadefinition_from_yaml(sectorfile, yaml_sector)
        overpasses = predict_overpass_area_def(tlefile, sector_area_def, satellite_list,
