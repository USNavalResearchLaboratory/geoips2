...
''' Stable wrapper functions for geoips2 reader interfaces.
...
def is_valid_reader(reader_name):
    ''' Check that the requested reader function has the correct call signature.
        ...
        Reader functions currently defined in
            <geoips2_package>.readers.<reader_name>.<reader_name>
...
def get_reader(reader_name):
    ...
    See: geoips2.stable.reader.is_valid_reader for full list of supported reader function call signatures and return values

    Args:
        reader_name (str) : Desired reader function (ie, 'amsr2_ncdf', 'ahi_hsd', etc)

    Returns:
        (<reader function>) : Reader function, currently specified in <geoips2_package>.readers.<reader_name>.<reader_name>
    '''
...
def get_reader_type(reader_name):
    ...
    Returns:
        (str) : Reader type currently found in <geoips2_package>.readers.<reader_name>.reader_type
...
def list_readers_by_type():
    ...
    See geoips2_utils.is_valid_reader? for a list of available reader types and associated call signatures / return values.
    See geoips2_utils.get_reader(reader_name) to retrieve the requested reader
...
def test_reader_interface():
    ''' Finds and opens every reader func available within the current geoips2 instantiation

    See geoips2_utils.is_valid_reader? for a list of available reader types and associated call signatures / return values.
    See geoips2_utils.get_reader(reader_name) to retrieve the requested reader

    Returns:
        (list) : List of all successfully opened geoips2 readers
    '''
