...
def area_def2mapobj(area_def):
    ...
    try:
        ...
        mapobj = area_def2basemap(area_def,
                                  fix_aspect=False,
                                  suppress_ticks=True,
                                  resolution='i')
    except ImportError:
...
def meridians(area_def, grid_size):
    ...
    LOG.info('List of meridians: min/max %s, %s', meridians_to_draw[0], meridians_to_draw[-1])
...
def check_gridlines_info_dict(gridlines_info):
    ...
    Args:
        gridlines_info (dict) : dictionary to check for required fields. For complete list of
                                  ...
                                  geoips2.image_utils.maps.get_gridlines_info_dict
    Returns:
    ...
    required_fields = ['grid_lat_spacing', 'grid_lon_spacing', 'grid_lat_fontsize', 'grid_lon_fontsize',
                       ...
                       'grid_lat_linewidth', 'grid_lon_linewidth',
                       'grid_lat_color', 'grid_lon_color',
                       'grid_lat_dashes', 'grid_lon_dashes']
    for key in required_fields:
        if key not in gridlines_info:
            raise ValueError('Missing gridlines_info entry {0}, required_fields {1}'.format(key,
                                                                                            required_fields))
...
def check_boundaries_info_dict(boundaries_info):
    ...
    Args:
        boundaries_info (dict) : dictionary to check for required fields. For complete list of
                                    fields, and appropriate defaults, see
                                    geoips2.image_utils.maps.get_boundaries_info_dict
    Returns:
        (Bool) : True if all fields are included, False if any fields are missing.
    '''
    required_fields = ['request_coastlines', 'coastlines_color', 'coastlines_linewidth',
                       ...
                       'request_countries', 'countries_color', 'countries_linewidth']
    for key in required_fields:
        if key not in boundaries_info:
            raise ValueError('Missing boundaries_info entry {0}, required_fields {1}'.format(key,
                                                                                             required_fields))
...
def draw_boundaries(mapobj, curr_ax, boundaries_info):
    ...
    Args:
        ...
        boundaries_info (dict) : Dictionary of parameters for plotting map boundaries.
                                 See geoips2.image_utils.maps.check_boundaries_info_dict
    ...
    if is_crs(mapobj):
        ...
        if boundaries_info['request_coastlines']:
            curr_ax.add_feature(cfeature.COASTLINE,
...
def draw_gridlines(mapobj, area_def, curr_ax, gridlines_info):
    ...
    if is_crs(mapobj):
        ...
        glnr = curr_ax.gridlines(color=gridlines_info['grid_lat_color'],
                                 draw_labels=True,
        ...
        LOG.info('    Plotting with basemap')
        mapobj.drawparallels(parallels(area_def, gridlines_info['grid_lat_spacing']),
                             ax=curr_ax,
                             linewidth=gridlines_info['grid_lat_linewidth'],
                             dashes=gridlines_info['grid_lat_dashes'],
                             labels=[gridlines_info['left_label'],
                                     gridlines_info['right_label'],
                                     0, 0],
        ...
        mapobj.drawmeridians(meridians(area_def, gridlines_info['grid_lon_spacing']),
                             ax=curr_ax,
                             linewidth=gridlines_info['grid_lon_linewidth'],
                             dashes=gridlines_info['grid_lon_dashes'],
                             labels=[0, 0,
                                     gridlines_info['top_label'],
                                     gridlines_info['bottom_label']],
