...
''' Test script for running representative products using data and comparison outputs from geoips_test_data_ repo '''

import logging
LOG = logging.getLogger(__name__)


def autotest_mint(fnames, command_line_args=None):
    ''' Pass through script to take command line arguments, and create calls to all required drivers
    ...
    from geoips2.drivers.pmw_mint_driver import pmw_mint_driver
    ...
    for fname in fnames:
        ...
        if command_line_args['product_options'] and 'includetemplate' in command_line_args['product_options']:
            ...
            command_line_args['readername'] = 'mint_ncdf'
            output_products += template_driver([fname], command_line_args)

        output_products += pmw_mint_driver([fname], command_line_args)
