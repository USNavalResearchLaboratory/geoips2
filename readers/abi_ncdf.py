...
def get_latitude_longitude(metadata,sect=None):
    '''
...
def get_satellite_angles(metadata, lats, lons, sect=None):
    # If the filename format needs to change for the pre-generated geolocation
    # files, please discuss prior to changing.  It will force recreation of all
    # files, which can be problematic for large numbers of sectors
...
def get_indexes(metadata, lats, lons, area_def):
    ...
    if not os.path.isfile(fname):
        ...
        fldk_ad = SwathDefinition(lons, np.ma.masked_less(lats, -999.1))
        ad = area_def
...
def abi_ncdf(fnames, metadata_only=False, chans=None, area_def=None, self_register=False):
    ...
    for fname in fnames:
        if chans:
            gotone = False
            for chan in chans:
                currchan = chan.replace('BT', '').replace('Ref', '').replace('Rad', '').replace('B', 'C')
    ...
    # G16 -> goes16
    xarray_obj.attrs['platform_name'] = highest_md['file_info']['platform_ID'].replace('G', 'goes')
    ...
    if area_def:
        ...
        xarray_obj.attrs['area_def'] = None
    ...
    if self_register:
        log.info('')
        log.info('Getting geolocation information for resolution {} for {}.'.format(
        ...
        for res in ['HIGH', 'MED', 'LOW']:
            try:
                ...
                continue
            try:
                gvars[res] = get_geolocation(sdt, res_md[res], area_def)
            except ValueError as resp:
    ...
    for chan, types in chan_info.items():
        ...
        for res, res_chans in DATASET_INFO.items():
            if chan in res_chans:
                break
        if (not self_register) and (res not in gvars.keys() or not gvars[res] or 'latitude' not in gvars[res]):
    ...
    for dsname in datavars.keys():
        ...
        if hasattr(xobj, 'area_def') and xobj.area_def is not None:
