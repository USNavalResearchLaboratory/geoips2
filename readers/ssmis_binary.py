...
def ssmis_binary(fnames, metadata_only=False, chans=False):
    ...
    if 'cfnoc' in os.path.basename(fname) and 'sdris' in os.path.basename(fname) :
        print('found a SSMIS SDR file')
    else:
        print('not a SSMIS SDR file: skip it')
        raise
...
#       -------- Apply the GEOIPS2 framework in XARRAY data frame ----------
        ...
        for ii in range(nscan_imager):
            if start_scantime_imager[ii] == -999:
                print('value of imager scan is {0}'.format(ii))
                continue
        ...
        for ii in range(nscan_enviro):
            if ii%2==0:                   #for odd scan numbers
                if start_scantime_enviro[ii] == -999:
                    print('value of enviro odd scan is {0}'.format(ii))
                    continue
            ...
            if ii%2==1:     # for even scan numbers (ch15_5x5, ch16_5x5, ch17_5x5, ch18_5x5, ch17_5x4, ch18_5x4 ??? need a review)
                if start_scantime_enviro[ii] == -999:
                    print('value of enviro even scan is {0}'.format(ii))
                    continue
                for jj in range(scenecounts_enviro[ii]):
...
#           Process LAS data record
        ...
        for ii in range(nscan_las):
            if start_scantime_las[ii] == -999:
                print('value of las scan is {0}'.format(ii))
                continue
...
#          Process UAS data record
        ...
        for ii in range(nscan_uas):
            if start_scantime_uas[ii] == -999:
                print('value of uas scan is {0}'.format(ii))
                continue
...
#       fill up space       
    ...
    print('start_time, end_time= {0} {1}'.format(start_time, end_time))

#--------------------- Xarray Objects for Processing Datasets--------------------------------------------------
#   conversion of TBs to K
#             TBs/100 + 273.15   (K)
#             setup xarray objects
    namelist_imager = ['latitude', 'longitude', 'V150', 'H183-7','H183-3', 'H183-1', 'V91', 'H91', 'sfcType','rain','timestamp']
...
# Setup attributes
    ...
    xarray_imager.attrs['original_source_filename'] = fname
    ...
    xarray_enviro.attrs['original_source_filename'] = fname
    ...
    xarray_las.attrs['original_source_filename'] = fname
    ...
    xarray_uas.attrs['original_source_filename'] = fname

    # MTIFs need to be "prettier" for PMW products, so 2km resolution for final image
    xarray_uas.attrs['sample_distance_km'] = 2
    xarray_uas.attrs['interpolation_radius_of_influence'] = 50000
 
    return [xarray_imager, xarray_enviro, xarray_las, xarray_uas]
