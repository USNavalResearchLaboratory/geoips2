...
def saphir_hdf(fnames, metadata_only=False, chans=False):
    ...
    londata = float(lon_obj.attrs['scale_factor']) * (lon_obj.value) + float(lon_obj.attrs['add_offset'])
    lat_obj = fileobj['ScienceData']['Latitude_Samples']
    # Offset is -40.0, scale_factor is 0.01
    latdata = float(lat_obj.attrs['scale_factor']) * (lat_obj.value) + float(lat_obj.attrs['add_offset'])

    date_time_group = fileobj['ScienceData']['Scan_FirstSampleAcqTime'].value
    ...
    ch1qf = np.ma.masked_equal(np.squeeze((fileobj['ScienceData']['QF_Samples_S1'].value)), 65535)
    ch2qf = np.ma.masked_equal(np.squeeze((fileobj['ScienceData']['QF_Samples_S2'].value)), 65535)
    ch3qf = np.ma.masked_equal(np.squeeze((fileobj['ScienceData']['QF_Samples_S3'].value)), 65535)
    ch4qf = np.ma.masked_equal(np.squeeze((fileobj['ScienceData']['QF_Samples_S4'].value)), 65535)
    ch5qf = np.ma.masked_equal(np.squeeze((fileobj['ScienceData']['QF_Samples_S5'].value)), 65535)
    ch6qf = np.ma.masked_equal(np.squeeze((fileobj['ScienceData']['QF_Samples_S6'].value)), 65535)
    ch1 = np.ma.masked_equal(np.squeeze(0.01*(fileobj['ScienceData']['TB_Samples_S1'].value)), 65535)
    ch2 = np.ma.masked_equal(np.squeeze(0.01*(fileobj['ScienceData']['TB_Samples_S2'].value)), 65535)
    ch3 = np.ma.masked_equal(np.squeeze(0.01*(fileobj['ScienceData']['TB_Samples_S3'].value)), 65535)
    ch4 = np.ma.masked_equal(np.squeeze(0.01*(fileobj['ScienceData']['TB_Samples_S4'].value)), 65535)
    ch5 = np.ma.masked_equal(np.squeeze(0.01*(fileobj['ScienceData']['TB_Samples_S5'].value)), 65535)
    ch6 = np.ma.masked_equal(np.squeeze(0.01*(fileobj['ScienceData']['TB_Samples_S6'].value)), 65535)
    scanqf = np.ma.masked_equal(np.squeeze((fileobj['ScienceData']['SAPHIR_QF_scan'].value)), 65535)
