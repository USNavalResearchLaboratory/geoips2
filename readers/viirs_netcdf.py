...
''' This VIIRS reader is designed for reading the NPP/JPSS VIIRS files geoips2.  The reader is only 
 ...
 V1.0:  NRL-Monterey, 09/17/2020 
  ...
  The xarray of geoips2 reader need both the data and lat/lon info. Thus, this VIIRS reader is designed to read in the paired
...
def viirs_netcdf(fnames, metadata_only=False, chans=None):
    ...
    for fname in fnames:
        ...
        if ncdf_file.platform == 'JPSS-1':
            xarrays[data_type].attrs['platform_name']  = 'JPSS1'
        xarrays[data_type].attrs['data_provider']  = 'NASA'
        xarrays[data_type].attrs['original_source_filename'] = os.path.basename(fname)
        
        # MTIFs need to be "prettier" for PMW products, so 2km resolution for final image
        xarrays[data_type].attrs['sample_distance_km'] = 2
        xarrays[data_type].attrs['interpolation_radius_of_influence'] = 10000
    ...
    for dtype in xarrays.keys():
        ...
        if 'DNBRad' in xarrays[dtype].variables.keys() and required_chan(chans, ['DNBRef']):
            try: 
                from geoips.geoalgs import lunarref
                xarrays['DNB']['DNBRef'] = xr.DataArray(numpy.ma.masked_array(lunarref(xarrays['DNB']['DNBRad'].to_masked_array(),
