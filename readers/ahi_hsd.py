...
def get_latitude_longitude(metadata,area_def=None):
    '''
    ...
    if not os.path.isfile(fname):
        if area_def is not None and DONT_AUTOGEN_GEOLOCATION and 'tc2019' not in area_def.area_id:
...
def get_satellite_angles(metadata, lats, lons, area_def=None):
...
def get_indexes(metadata, lats, lons, area_def):
    ...
    fname = get_geolocation_cache_filename('GEOLL', metadata, area_def)

    if not os.path.isfile(fname):
        if area_def is not None and DONT_AUTOGEN_GEOLOCATION and 'tc2019' not in area_def.area_id:
            msg = ('GETGEO Requested NO AUTOGEN GEOLOCATION. ' +
                   'Could not create inds_file {} for {}').format(fname, area_def.area_id)
            log.error(msg)
            raise AutoGenError(msg)
        # Allocate the full disk area definition
        log.info('    GETGEOINDS Creating full disk swath definition for {}'.format(area_def.area_id))
        ...
        log.info('    GETGEOINDS Running get_neighbour_info {}'.format(area_def.area_id))
        ...
        try:
            ...
            with open(str(fname), 'w') as df:
                log.warning('No coverage, generating empty GEOINDS file %s for sector %s', fname, area_def.area_id)
            raise IndexError(resp)

        log.info('    GETGEOINDS Writing to {} : inds_file for {}'.format(fname, area_def.area_id))
        with open(str(fname), 'w') as df:
            lines.tofile(df)
            samples.tofile(df)
    ...
    try:
        ...
        log.warning('Mismatched geolocation file size (Empty?  No coverage?  Or old sector of different shape?')
        raise IndexError(resp)
    # Switch to xarray based geolocation files
    # saved_xarray = xarray.load_dataset(fname)
...
def get_geolocation_cache_filename(pref, metadata, area_def=None):
    ...
    # from hashlib import sha1
    # md_hash = sha1(frozenset((k, v) for k, v in metadata.items() if isinstance(v, Hashable)).__str__().encode('ascii')).hexdigest()
    ...
    # If the filename format needs to change for the pre-generated geolocation
    # files, please discuss prior to changing.  It will force recreation of all
    # files, which can be problematic for large numbers of area_defors
...
def get_geolocation(dt, metadata, area_def=None):
    '''
    ...
    try:
        fldk_lats, fldk_lons = get_latitude_longitude(gmd,area_def)
        fldk_sat_zen, fldk_sat_azm = get_satellite_angles(gmd, fldk_lats, fldk_lons,area_def)
    except AutoGenError:
        return False
    ...
    if area_def is not None:
        try:
            ...
            log.warning('%s No coverage?', str(resp))
            raise IndexError(resp)
...
def _get_metadata_block_info(df):
    ...
    for blockind in range(1, 12):
        ...
        if block_num != blockind:
            raise IOError('Unexpected block number encountered.  Expected %s, but got %s. File %s' %
...
def ahi_hsd(fnames, metadata_only=False, chans=None, area_def=None, self_register=False):
        ...
        if chans:
            ...
            for chan in chans:
                ...
                if currchan in fname:
                    gotone = True
            if not gotone:
                log.info('SKIPPING file %s, not needed from channel list %s', fname, chans)
                continue
        try:
    ...
    xarray_obj.attrs['platform_name'] = highest_md['block_01']['satellite_name'].replace('-', '').lower()
    xarray_obj.attrs['area_def'] = area_def
    ...
    if self_register:
        ...
        gvars[adname] = get_geolocation(dt, res_md[self_register], area_def)
        if not gvars[adname]:
        ...
        for res in ['HIGH', 'MED', 'LOW']:
            try:
                ...
                continue
            log.info('')
            log.info('Getting geolocation information for resolution %s for %s', res, adname)
            try:
                gvars[res] = get_geolocation(dt, res_md[res], area_def)
            except IndexError as resp:
    ...
    for chan, types in chan_info.items():
        ...
        if (not self_register) and (res not in gvars.keys() or not gvars[res]):
            log.info("We don't have geolocation information for {} for {} skipping {}".format(
                res, adname, chan))
            continue
        if not area_def:
            dsname = res
    ...
    for dsname in datavars.keys():
        ...
        if hasattr(xobj.attrs, 'area_def') and xobj.area_def is not None:
            xobj.attrs['interpolation_radius_of_influence'] = max(xobj.area_def.pixel_size_x, xobj.area_def.pixel_size_y)
        else:
